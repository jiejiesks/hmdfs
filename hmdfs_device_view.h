...
static inline void hmdfs_get_lower_path(struct dentry *dent, struct path *pname)
{
	...
	pname->dentry = hmdfs_d(dent)->lower_path.dentry;//hmdfs_d(dent)返回的是hmdfs_dentry_info *，其成员有lower_path，lower_path成员有dentry *dentry;
	pname->mnt = hmdfs_d(dent)->lower_path.mnt;//lower_path的vfsmount *mnt成员
	path_get(pname);
...
int init_hmdfs_dentry_info(struct hmdfs_sb_info *sbi, struct dentry *dentry,
			   int dentry_type);//初始化init_hmdfs_dentry_info，其调用了hmdfs_set_lower_path
