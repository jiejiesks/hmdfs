...
int hmdfs_file_open_local(struct inode *inode, struct file *file)
{
	...
	struct super_block *sb = inode->i_sb;//获取hmdfs的sb
	...
	hmdfs_get_lower_path(file->f_path.dentry, &lower_path);//获取lower_path，即把挂在信息dentry和vfsmount放入lower_path
	lower_file = dentry_open(&lower_path, file->f_flags, cred);
	hmdfs_put_lower_path(&lower_path);//释放lower_path
	if (IS_ERR(lower_file)) {
		err = PTR_ERR(lower_file);
		kfree(gfi);
	} 

	if (lower_file->f_inode->i_flags & S_DEDUP) {
		kernel_read(lower_file, deduppath, sizeof(deduppath), &rpos);
		hmdfs_info("path: %s", deduppath);
		//in release fput readfile,filp_close call fput
		readfile = filp_open(deduppath, O_RDWR | O_CREAT, 0);
		if (IS_ERR(readfile)) {
			hmdfs_err("read failed");
			return -1;
		}
		gfi->lower_file = readfile;
