...
static int hmdfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (err)
		goto out_freesbi;
//完成vfs的sb指向hmdfs的sb的初始化
	sb->s_fs_info = sbi;//vfs的sb的s_fs_info指向hmdfs_sb_info内存映像
	...
	err = kern_path(dev_name, LOOKUP_FOLLOW | LOOKUP_DIRECTORY,
			&lower_path);//获取挂在路径的信息有dentry和vfsmount，存储在lower_path中，
	if (err) {
		hmdfs_err("open dev failed, errno = %d", err);
		goto out_unreg_sysfs;
	}
	//通过lower_path获取到下层文佳系统的sb
	lower_sb = lower_path.dentry->d_sb;
	atomic_inc(&lower_sb->s_active);
	sbi->lower_sb = lower_sb;//hmdfs的sb的lower_sb指向下层文件系统的sb
	...
	root_inode = fill_root_inode(sb, d_inode(lower_path.dentry));//获取挂载目录的hmdfs层的inode
	if (IS_ERR(root_inode)) {
		err = PTR_ERR(root_inode);
		goto out_sput;
	}
	hmdfs_root_inode_perm_init(root_inode);
	sb->s_root = root_dentry = d_make_root(root_inode);//设置全局根目录
	if (!root_dentry) {
		err = -ENOMEM;
		goto out_sput;
	}

	err = init_hmdfs_dentry_info(sbi, root_dentry, HMDFS_LAYER_ZERO);//初始化hmdfs_dentry_info
	if (err)
		goto out_freeroot;
...
static int __init hmdfs_init(void)//驱动入口函数
{
	int err = 0;
	...
	err = register_filesystem(&hmdfs_fs_type);//注册文件系统
	if (err) {
